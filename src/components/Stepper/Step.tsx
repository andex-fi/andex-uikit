...
const Connector = styled.div<StatusProps>`
  ...
  background-color: ${({ theme, status }) =>
    theme.colors[status === "past" || status === "current" ? "universal" : "textDisabled"]};
`;
...
export const StepNumber = styled.div<StatusProps>`
  ...
  background-color: ${({ theme, status }) => theme.colors[status === "current" ? "universal" : "invertedContrast"]};
  border: 2px solid ${({ theme, status }) => (status === "past" ? theme.colors.universal : "transparent")};
